...
const AssetListItem = ({ name, symbol, price, change, isPositive, image }) => {
	return (
		<div className="flex items-center justify-between py-3 gap-2">
			<div className="flex items-center gap-3 min-w-0 flex-1">
				<img src={image} alt={symbol} className="w-8 h-8 rounded-full bg-gray-10 shrink-0" />
				<div className="min-w-0">
					<h4 className="font-medium text-gray-100 truncate">{name}</h4>
					<p className="text-gray-60 text-sm uppercase">{symbol}</p>
				</div>
			</div>
			<div className="flex items-center gap-4 shrink-0">
				<div className="text-right">
					<p className="font-medium text-gray-100">${price.toLocaleString()}</p>
					<div className="flex justify-end mt-0.5">
						<ChangeIndicator value={change} isPositive={isPositive} />
					</div>
				</div>
				<button className="bg-blue-60 hover:bg-[#0045D8] transition-colors text-white font-medium px-4 py-2 rounded-full text-sm shrink-0">
					Trade
				</button>
			</div>
		</div>
			...
			<div className="flex flex-col gap-2">
				{assets.map((asset) => (
					<AssetListItem
						...
						symbol={asset.symbol}
						price={asset.price}
						change={parseFloat(asset.change24h.replace('+', ''))}
...
export default function MarketStatsPage() {
	...
	const stats = useMemo(() => {
		if (coins.length === 0) return { totalMktCap: 0, totalVol: 0, btcDom: 0, btcChange: 0 };
		const totalMktCap = coins.reduce((s, c) => s + (c.price * 1000000), 0);
		const totalVol = coins.reduce((s, c) => s + (c.price * 50000), 0);
		const btc = coins.find(c => c.symbol === 'BTC');
		const btcDom = btc ? (btc.price * 1000000 / totalMktCap * 100) : 0;
		const btcChange = btc ? parseFloat(btc.change24h.replace('+', '')) : 0;
		return { totalMktCap, totalVol, btcDom, btcChange };
	}, [coins]);
	...
	const MetricRow = ({ title, description, value, change, isPositive, listTitle, assets }) => (
		<div className="border-b border-gray-10 last:border-0 hover:bg-gray-5/30 transition-colors">
			<div className="max-w-[1680px] mx-auto px-6 grid grid-cols-1 md:grid-cols-12 min-h-[300px]">
				{/* Left Column (Info) */}
			...
			<main className="flex-1 w-full pt-16">
				...
				<div className="w-full border-t border-gray-10">
					...
					<MetricRow
						...
						value={`$${(stats.totalMktCap / 1e12).toFixed(2)}T`}
						change={-1.09}
						isPositive={false}
						listTitle="Top Assets"
						assets={coins.slice(0, 3)}
					/>
					...
					<MetricRow
						...
						value={`$${(stats.totalVol / 1e12).toFixed(2)}T`}
						change={4.38}
						isPositive={true}
						listTitle="Most volume"
						assets={coins.slice(3, 6)}
					/>
					...
					<MetricRow
						...
						value={`${stats.btcDom.toFixed(2)}%`}
						change={stats.btcChange}
						isPositive={stats.btcChange >= 0}
						listTitle="Top Assets"
						assets={coins.slice(0, 3)}
					/>
