function VerifyingAddressStep({ onFinish }) {
	useEffect(() => {
	...
	return (
		<div className="min-h-screen bg-[#0A0B0D] flex flex-col items-center justify-center px-4">
			<div className="w-full max-w-lg mx-auto">
				...
				<p className="text-[#8A919E] mb-6">Make sure that the address listed on your document and the address you entered on CryptoApp match exactly.</p>
...
/* ════════════════════════════════════════════════════
   ...
   ════════════════════════════════════════════════════ */
const StepEmail = ({ name, setName, email, setEmail, password, setPassword, onNext, error }) => (
	<Shell>
		<form onSubmit={(e) => { e.preventDefault(); if (name && email && password) onNext(); }}>
			...
			<p className="text-[0.9375rem] text-[#8A919E] mb-6 leading-6">
				Access all that CryptoApp has to offer with a single account.
			</p>
			...
			<DarkInput label="Password" type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Create a password" />
...
const StepVerifyEmail = ({ email, onNext }) => {
	...
	const [mockCode] = useState(() => Math.floor(100000 + Math.random() * 900000).toString());
	const refs = useRef([]);
	...
	return (
		<Shell>
			...
			<div className="mb-6 p-4 bg-blue-500/10 border border-blue-500/20 rounded-xl">
				<p className="text-blue-400 text-sm font-medium mb-1">Mock Verification Code (Dev Mode):</p>
				<p className="text-white text-2xl font-mono tracking-widest">{mockCode}</p>
			</div>
	...
	<Shell>
		<div className="flex flex-col items-center text-center">
			...
			<p className="text-[0.9375rem] text-[#8A919E] mb-4 leading-6 max-w-sm">
				Would you like to receive emails from CryptoApp on price updates, product announcements, and trading insights we think you might be interested in?
			</p>
...
const StepCountry = ({ citizenship, setCitizenship, residence, setResidence, onNext }) => (
	<Shell>
		<form onSubmit={(e) => { e.preventDefault(); onNext(); }}>
			...
			<p className="text-[0.9375rem] text-[#8A919E] mb-6 leading-6">
				CryptoApp is <a href="#" className="underline text-white hover:text-[#0052FF]">legally required</a> to collect this information. If you&apos;re a citizen of more than one country, please pick one.
			</p>
...
const StepBirth = ({ city, setCity, country, setCountry, onNext }) => (
	<Shell>
		<form onSubmit={(e) => { e.preventDefault(); onNext(); }}>
			...
			<p className="text-[0.9375rem] text-[#8A919E] mb-6 leading-6">
				CryptoApp is legally required to collect this info.
			</p>
...
/* ════════════════════════════════════════════════════
   ...
   ════════════════════════════════════════════════════ */
const StepVerifying = ({ onNext }) => {
	useEffect(() => {
		const id = setTimeout(onNext, 2500);
		return () => clearTimeout(id);
	}, [onNext]);
...
const SignUp = () => {
	...
	const handleRegister = async () => {
		...
		try {
			...
			setStep(1); // Move to email verification step
		} catch (err) {
	...
	switch (step) {
		case 0: return <StepEmail name={name} setName={setName} email={email} setEmail={setEmail} password={password} setPassword={setPassword} onNext={handleRegister} error={error} />;
		...
		case 8: return <StepVerifying onNext={next} />;
		...
		case 12: return <VerifyingAddressStep onFinish={() => window.location.href = '/profile'} />;
